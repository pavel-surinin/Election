  ...
  for (var {{=$key}} in {{=$data}}) {
    ...
    {{??}}
      ...
      {{?? $additionalIsSchema }}
        ...
        {{??}}
          {{# def.validateAdditional }}
          {{? $breakOnError }} if (!validCount{{=$it.level}}) break; {{?}}
        {{?}}
      {{?}}
...
{{~ $pPropertyKeys:$pProperty }}
  ...
  {{? {{# def.nonEmptySchema:$sch}} }}
    ...
    for (var {{=$key}} in {{=$data}}) {
      ...
      {{? $breakOnError }} else validCount{{=$it.level}} = true; {{?}}
    }
    ...
    {{? {{# def.nonEmptySchema:$sch}} }}
      ...
      {{? $pgMin !== undefined || $pgMax !== undefined }}
        ...
        {{? $pgMin !== undefined }}
          ...
          {{=$validCount}} = pgPropCount{{=$lvl}} >= {{=$pgMin}};
        ...
        {{? $pgMax !== undefined }}
          ...
          {{=$validCount}} = pgPropCount{{=$lvl}} <= {{=$pgMax}};
