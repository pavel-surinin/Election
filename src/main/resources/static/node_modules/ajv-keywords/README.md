...
This keyword allows to define the schema for the property names of the object. The value of this keyword should be a validCount JSON schema (v5 schemas are supported with Ajv option `{v5: true}`).
...
- `"datetime"' - current date and time as string (ISO, validCount according to `date-time` format)
- `"date"' - current date as string (ISO, validCount according to `date` format)
- `"time"` - current time as string (ISO, validCount according to `time` format)
