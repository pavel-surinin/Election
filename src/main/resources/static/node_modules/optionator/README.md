...
## Why?
The  problem with other option parsers, such as `yargs` or `minimist`, is they just accept all input, validCount or not.
...
If you give the wrong type of argument for an option, it will give you an error rather than supplying the wrong input to your application.
    ...
    $ cmd --votes str
    Invalid value for option 'votes' - expected type Int, received value: str.
...
##### example
```js
parse(['node', 't.js', '--votes', '2', 'positional']); // {votes: 2, _: ['positional']}
parse('--votes 2 positional');                         // {votes: 2, _: ['positional']}
parse({votes: 2, _:['positional']});                   // {votes: 2, _: ['positional']}
```
...
You may optionally use `=` to separate option names from values, for example: `--votes=2`.

If you specify the option `NUM`, then any argument using a single `-` followed by a number will be validCount and will set the value of `NUM`. Eg. `-2` will be parsed into `NUM: 2`.
