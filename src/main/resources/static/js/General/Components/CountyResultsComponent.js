...
var CountyResultsComponent = React.createClass({
  ...
  render: function(){
    ...
    if (county.name) {
      countyInfo.push(<tr>
        <td>{county.name} apygarda</td>
      </tr>);
    }
    if (countyResults.districtsCount) {
      countyInfo.push(<tr>
        <td>Apylinkių skaičius: {countyResults.districtsCount}</td>
      </tr>)
    }
    if (countyResults.districtsVotedSingle) {
      countyInfo.push(<tr>
        <td>Vienmandatėje prabalsavo apylinkių: {countyResults.districtsVotedSingle} </td>
      </tr>)
    }
    if (countyResults.singleMandateWinner) {
      countyInfo.push(<tr>
        <td>Vienmandatės
          nugalėtojas: {countyResults.singleMandateWinner.name} {countyResults.singleMandateWinner.surname}</td>
      </tr>)
    }
    if (countyResults.districtsVotedMulti) {
      countyInfo.push(<tr>
        <td>Daugiamandatėje prabalsavo apylinkių: {countyResults.districtsVotedMulti} </td>
      </tr>)
    }
    if (countyResults.districtsVotedSingle) {
      countyInfo.push(<tr>
        <td>// TODO : AWESOME PROGRESS BAR ////////////---- %</td>
      </tr>)
    }
